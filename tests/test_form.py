...
from starlite.datastructures import ImmutableMultiDict
...
def test_populate_from_get_request(app, client, BasicForm):
    ...
    async def index(request):
        ...
        assert form.avatar.data is None
        assert form.checkbox.data is True
...
def test_populate_from_post_request_files(app, client, BasicForm):
    ...
    async def index(request):
        ...
        assert form.avatar.data.filename == "starlite.png"
        return PlainTextResponse()
...
def test_populate_from_post_request_json(app, client, BasicForm):
    ...
    client.post("/", data=json.dumps({"name": "json"}), headers={"content-type": "application/json"})
