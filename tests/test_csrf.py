...
from starlite import HTTPEndpoint
...
from starlite.template import Jinja2Templates
from starlite.testing import TestClient

from starlite_wtf import CSRFProtectMiddleware, StarliteForm, csrf_protect, csrf_token
...
def test_disabled_by_default(app, client):
    ...
    async def index(request):
        form = StarliteForm(request)
        assert hasattr(form, "csrf_token") is False
        return PlainTextResponse()
...
def test_enabled_false():
    ...
    async def endpoint_without_decorator(request):
        form = await StarliteForm.from_formdata(request)
    ...
    response = client.post("/without-decorator", data={"csrf_token": "badT0ken"})
    assert response.status_code == 200
    assert response.text == "SUCCESS"
...
def test_wtf_form_handling_with_decorator(make_csrf_app, BasicForm):
    ...
    async def endpoint(request):
        ...
        text = "FORM" if request.method == "GET" else "FAIL"
    ...
    response = client.post("/endpoint", data={"csrf_token": "badt0ken", "mykey": "myval"})
    assert response.status_code == 403
    ...
    response = client.post("/endpoint", data={"csrf_token": signed_token, "name": "myval"})
    assert response.status_code == 200
    assert response.text == "SUCCESS"
...
def test_raw_form_handling_with_decorator(make_csrf_app):
    ...
    signed_token = client.get("/token").text
    response = client.post("/endpoint", data={"mykey": "myval", "csrf_token": signed_token})
    assert response.status_code == 200
...
def test_csrf_valid_flag(make_csrf_app):
    ...
    async def endpoint(request):
        if request.method == "POST":
            assert hasattr(request.state, "csrf_valid") is True
        return PlainTextResponse()
...
def test_csrf_ssl_strict(make_csrf_app):
    ...
    response = client.post(
        "https://testserver/endpoint", headers={"REFERER": "https://testserver/"}, data={"csrf_token": signed_token}
    )
    assert response.status_code == 200

    # different scheme
    response = client.post(
        "https://testserver/endpoint", headers={"REFERER": "http://testserver/"}, data={"csrf_token": signed_token}
    )
    assert response.status_code == 403

    # different hostname
    response = client.post(
        "https://testserver/endpoint", headers={"REFERER": "https://testserver2/"}, data={"csrf_token": signed_token}
    )
    assert response.status_code == 403

    # different port
    response = client.post(
        "https://testserver/endpoint",
